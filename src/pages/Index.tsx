...
const Index = () => {
  useSeoMeta({
    title: 'NostrMusic - Discover Bitcoin Music on Nostr',
    description: 'Discover trending Bitcoin music, search artists and albums, and explore the decentralized music ecosystem.',
  });
  ...
  const { data: artistData, isLoading: _isLoadingArtist } = useWavlakeArtist(selectedArtistId || undefined);
  const { data: albumData, isLoading: _isLoadingAlbum } = useWavlakeAlbum(selectedAlbumId || undefined);
    ...
    try {
      ...
      queryClient.setQueryData(['wavlake-picks', PEACHY_PUBKEY], (oldData: unknown) => {
        if (!oldData || typeof oldData !== 'object' || !('tracks' in oldData)) {
          return {
            tracks: [track],
            title: "NostrMusic Weekly Wavlake Picks",
